...
public class DeleteConcluirCompraTest {
    ...
    @Test
    @DisplayName("Cenário 40: Concluir compra com sucesso (Positivo)")
    public void deveConcluirCompraComSucesso() {
        ...
        carrinhoClient.concluirCompra(tokenUsuario)
                .then()
    ...
    @Test
    @DisplayName("Cenário 41: Tentar concluir compra sem possuir carrinho (Positivo/Idempotente)")
    public void deveTentarConcluirCompraSemCarrinho() {
        // 1. Criar Usuário e Logar (sem criar carrinho)
        UsuarioModel usuario = UsuarioDataFactory.criarUsuarioNaoAdmin();
        usuarioClient.cadastrarUsuario(usuario).then().statusCode(201);
        String tokenUsuario = loginClient.realizarLogin(UsuarioModel.builder()
                        .email(usuario.getEmail()).password(usuario.getPassword()).build())
    ...
    @Test
    @DisplayName("Cenário 42: Tentar concluir compra sem Token de acesso (Negativo)")
    public void naoDeveConcluirCompraSemToken() {
        carrinhoClient.concluirCompra("Bearer token_invalido_123")
                .then()
                .log().ifValidationFails()
                .statusCode(401)
